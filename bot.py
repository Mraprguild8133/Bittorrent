...
class ProgressTracker:
    ...
    def __init__(self):
        ...
        self.update_interval = 3  # Update every 3 seconds minimum
    ...
    async def update_progress(self, current, total, client, message, start_time):
        ...
        should_update = (
            ...
            progress_since_last_update >= 15 or  # 15% progress
            percentage >= 99.5 or  # Final update
            current == 0 or  # Initial update
            percentage <= 5  # First 5%
        )
        ...
        time_left = (total - current) / speed if speed > 0 and current > 0 else 0
        ...
        if text == self.last_message_text and self.last_message_id == message.id:
            return
...
async def upload_to_gdtot(file_path, original_filename):
    """Upload file to GDToT using their API with proper Google Drive links."""
    ...
    try:
        # For GDToT API, we need to provide a direct download URL
        # Since we have the file locally, we need to upload it to a temporary service first
        # or use the actual GDToT file upload endpoint if available
        
        temp_download_url = await upload_to_temp_service(file_path, original_filename)
        
        if not temp_download_url:
            return "❌ **GDToT Transfer Failed:** Could not upload file to temporary service"
        
        # Prepare the API request for GDToT
        headers = {
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    if result.get('status') == 'success':
                        ...
                        # Generate proper Google Drive link
                        if file_id and not gdrive_link:
                    ...
                    return f"❌ **GDToT Transfer Failed:** Bad Request - {error_text}"
...
async def upload_to_temp_service(file_path, filename):
    """
    Upload file to a temporary service to get a direct download URL.
    This is required for GDToT API which expects a URL.
    """
    try:
        # For now, we'll use file.io as a temporary service
        # You can replace this with any file hosting service
        ...
        # Fallback: If file.io fails, try transfer.sh
        with open(file_path, 'rb') as file:
            files = {'file': file}
        ...
        print(f"Error uploading to temp service: {e}")
        return None
...
async def download_file_http(url, file_name, progress_msg, client):
    ...
    try:
        ...
        await progress_callback(0, total_size, client, progress_msg, start_time)
...
async def process_file(client, message, file_path, file_name, file_size):
    ...
    # Reset progress tracker for processing phase
    progress_tracker.reset()
    ...
    # Update message once with initial status
...
# [Rest of the message handling functions remain unchanged from previous code]
