...
async def upload_to_wasabi(client: Client, chat_id: int, file_path: str):
    ...
    try:
        status_msg = await client.send_message(chat_id, f"☁️ **Uploading to Wasabi...**\n`{wasabi_key}`")
        ...
        await status_msg.edit_text(
            ...
            f"**Link:** [Download]({wasabi_url})"
        )
        return True

    except Exception as e:
        logger.error(f"Wasabi upload error: {e}")
        await client.send_message(chat_id, f"❌ Upload failed: `{str(e)}`")
        return False
    finally:
        # Clean up local file
...
async def start_command(client: Client, message: Message):
    ...
    wasabi_status = "✅ Enabled" if WASABI_CLIENT else "❌ Disabled"
    
    await message.reply_text(
        f"👋 **Welcome to Media Handler Bot!**\n\n"
        f"**Status:**\n"
...
async def mirror_command(client: Client, message: Message):
    ...
    if not message.reply_to_message or not message.reply_to_message.media:
        await message.reply_text("❌ Please reply to a file with /mirror")
        return
    ...
    if media.video:
        ...
        file_name = media.document.file_name or "file.bin"
    elif media.audio:
    ...
    if not validate_file_size(file_size):
        await message.reply_text(
            ...
            f"Max size: {config.MAX_FILE_SIZE}MB"
        )
        return
