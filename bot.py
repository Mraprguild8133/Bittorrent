...
# Initialize bot without plugins parameter
bot = Client(
    "bot",
    api_id=API_ID,
    api_hash=API_HASH,
    ...
    try:
        ...
        for i in content:
            if i.strip():  # Skip empty lines
                # Split by :// but keep the protocol
                if "://" in i:
                    parts = i.split("://", 1)
    ...
    try:
        ...
        if "drive" in V:
            try:
                ...
                if use_wasabi:
                    success, new_count = await helper.send_doc(bot, m, cc, ka, cc1, None, count, name, use_wasabi=True)
                else:
                    success, new_count = await helper.send_doc(bot, m, cc, ka, cc1, None, count, name)
                if success:
                    count = new_count
                ...
                await m.reply_text(f"Error downloading: {str(e)}")
                return False, count
            ...
            try:
                ...
                if use_wasabi:
                    success, new_count = await helper.send_doc(bot, m, cc, f'{name}.pdf', cc1, None, count, name, use_wasabi=True)
                else:
...
async def upload_handler(bot: Client, m: Message):
    ...
    for i in range(start_index - 1, len(links)):
        ...
        success, new_count = await process_link(
...
async def ping_command(bot: Client, m: Message):
    start_time = time.time()
    msg = await m.reply_text("**Pinging...**")
    end_time = time.time()
    await msg.edit(f"**Pong!** 🏓\n`{round((end_time - start_time) * 1000, 2)} ms`")
